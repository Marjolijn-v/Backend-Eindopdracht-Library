...
public class LoanActivityDtoMapper implements DtoMapper<LoanActivityResponseDto, LoanActivityRequestDto, LoanActivityEntity> {
    ...
    public LoanActivityResponseDto mapToDto(LoanActivityEntity model) {
        ...
        result.setBook(mapBookToSummary(model.getBook()));
    ...
    public LoanActivityEntity mapToEntity(LoanActivityRequestDto loanActivityModel) {
        ...
        BookEntity book = bookRepository.findById(loanActivityModel.getBookId()).orElseThrow(() -> new RecordNotFoundException("Boek niet gevonden"));
        result.setBook(book);

        UserEntity user = userRepository.findById(loanActivityModel.getUserId()).orElseThrow(()-> new RecordNotFoundException("Gebruiker niet gevonden"));
        result.setUser(user);
    ...
    private BookSummaryDto mapBookToSummary(BookEntity book) {
        var result = new BookSummaryDto();
        result.setId(book.getId());
        result.setTitle(book.getTitle());
        result.setReleaseYear(book.getReleaseYear());
        result.setDescription(book.getDescription());
        result.setNumberOfCopies(book.getNumberOfCopies());
        return result;
    }
}
